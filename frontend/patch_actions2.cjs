...
text = text.replace(
  /\{item\.status === 'Por verificar' && \(\s*<div className="card p-4 bg-amber-50 border-amber-200">\s*<p className="font-semibold text-amber-900 mb-3">?? Solicitud pendiente de verificación<\/p>\s*<div className="flex gap-2">\s*<button className="btn bg-green-600 text-white hover:bg-green-700" onClick=\{onApprove\}>\s*? Verificar publicación\s*<\/button>\s*<button className="btn bg-red-600 text-white hover:bg-red-700" onClick=\{onReject\}>\s*? Rechazar\s*<\/button>\s*<button className="btn bg-slate-600 text-white hover:bg-slate-700" onClick=\{onReturnToDraft\}>\s*Devolver a Borrador\s*<\/button>\s*<\/div>\s*<\/div>\s*\)\}/,
