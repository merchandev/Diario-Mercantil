...
export default function Topbar() {
  ...
  const roleDisplayName = isAdminRole(user?.role) ? 'Admin' : 'Solicitante'
          ...
          <div className="ml-auto flex items-center gap-3">
            {user?.name && (
              <div className="hidden sm:flex items-center gap-2 text-sm text-slate-600">
                ...
                {user?.role && (
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${isAdminRole(user.role) ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                    {roleDisplayName}
                  </span>
                )}
              </div>
