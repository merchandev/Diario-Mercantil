...
export default function PublicHeader() {
  ...
  useEffect(() => {
    ...
    listPagesPublic().then(({ items }) => {
      ...
      if (!m.find(x => x.to === '/ediciones')) {
        ...
        else m.push({ label: 'EDICIONES', to: '/ediciones' })
      }
  ...
  return (
    <>
      <TopBannerRow />
      <nav className="bg-white/90 backdrop-blur border-y border-slate-200 sticky top-0 z-40 md:mt-4">
        <div className="mx-auto max-w-7xl px-4 min-h-12 flex items-center justify-between md:justify-start gap-4 py-2 md:py-0 relative">
          ...
          <div className={`${isMenuOpen ? 'flex' : 'hidden'} md:flex absolute md:relative top-full left-0 w-full md:flex-1 md:justify-center md:items-center bg-white/95 md:bg-transparent backdrop-blur md:backdrop-blur-none border-b md:border-b-0 border-slate-200 md:h-12 flex-col md:flex-row shadow-lg md:shadow-none p-4 md:p-0 gap-4 md:gap-6 z-50 transition-all origin-top`}>
            ...
            <div className="md:hidden border-t border-slate-200 my-2 pt-4">
              {user ? (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex gap-2">
                    <Link to={isAdminRole(user.role) ? "/dashboard" : "/solicitante"} onClick={() => setIsMenuOpen(false)} className="btn btn-primary h-9 flex-1 text-xs justify-center flex items-center gap-2">
          ...
          <div className="md:ml-auto hidden md:flex items-center gap-4 shrink-0 border-l border-slate-200 pl-4">
            {user ? (
              <>
                <div className="flex items-center gap-2 text-sm text-slate-600">
                  {user.avatar_url ? (
