...
function CornerCurl({
  ...
  const gradientAngle =
    isR && isBot ? "135deg" : isR ? "225deg" : isBot ? "45deg" : "315deg";
          ...
          return (
            <button
              ...
              style={{
                ...
                boxShadow: isActive
                  ? "0 0 0 1px #e63d3d, 0 8px 32px rgba(230,61,61,0.25)"
...
function PageCursor({
  ...
  return (
    ...
    >
      {side === "right" ? (
        <>
          <span>Arrastra para pasar</span>
          <span>→</span>
        </>
      ) : (
        <>
          <span>←</span>
          <span>Arrastra para regresar</span>
        </>
      )}
    </div>
  );
...
function FlipEngine({
  ...
  let pageW = isMobile
    ? Math.min(containerW - 16, 380)
    : Math.floor((containerW - 60) / 2);
  ...
  const animateTo = useCallback(
    (dir: "next" | "prev", fromAngle: number, toAngle: number) => {
      cancelRaf();
      const t0 = performance.now();
      const duration = Math.max(
        80,
        (Math.abs(toAngle - fromAngle) / 180) * 680,
      );
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const initDragY = Math.max(
        0,
        Math.min(1, (e.clientY - rect.top) / rect.height),
      );

      const startTrans: Trans = {
        dir,
        angle: 0,
        dragging: true,
        dragY: initDragY,
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        if (isMobile) {
          ...
          if (dir === "next") {
            ...
            angle = Math.max(
              0,
              Math.min(180, (traveled / capturedPageW) * 180),
            );
          } else {
        ...
        const dragY = Math.max(
          0,
          Math.min(1, (ev.clientY - capturedRect.top) / capturedRect.height),
        );
      ...
      const onUp = (ev: PointerEvent) => {
        ...
        try {
          bookRef.current?.releasePointerCapture(ev.pointerId);
        } catch (err) { }
  ...
  const curMaxPage = isMobile
    ? curPageNum
    : cur.right
      ? curPageNum + (cur.left ? 1 : 0)
      : curPageNum;
    ...
    >
      ...
      >
        <div
          ...
          onPointerMove={(e) => {
            ...
            setHoverHalf(
              e.clientX - rect.left > rect.width / (isMobile ? 1 : 2)
                ? "right"
                : "left",
            );
          }}
            ...
            if (singleRight || singleLeft) {
              ...
              return (
                ...
                >
                  ...
                  <PageCursor
                    side="right"
                    visible={canNext && hoverHalf === "right"}
                  />
                  <PageCursor
                    side="left"
                    visible={canPrev && hoverHalf === "left"}
                  />
                </div>
              );
            }
              ...
              <>
                <div style={{ position: "relative" }}>
                  ...
                  <CornerCurl
                    side="left"
                    corner="top"
                    visible={hoverHalf === "left" && spread > 0}
                  />
                  <CornerCurl
                    side="left"
                    corner="bottom"
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <CornerCurl
                    side="right"
                    corner="top"
                    visible={hoverHalf === "right" && spread < maxS}
                  />
                  <CornerCurl
                    side="right"
                    corner="bottom"
      ...
      <div
        style={{
          ...
          gap: isMobile ? 8 : 12,
          width: "100%",
          maxWidth: isMobile ? pageW + 30 : pageW * 2 + 60,
      ...
      >
        ...
        >
          ...
          >
            Pág.&nbsp;{curPageNum}
            {!isMobile && cur.left && cur.right ? `–${curPageNum + 1}` : ""}
            &nbsp;de&nbsp;{totalPages}
          </span>
          <div
            style={{
          ...
          >
            <div
              style={{
                height: "100%",
                width: `${pct}%`,
                background: "linear-gradient(90deg,#e63d3d,#ff6b6b)",
                borderRadius: 4,
        ...
        <NavBtn
          isMobile={isMobile}
          dir="next"
          disabled={spread >= maxS || isFlipping}
          onClick={() => doFlip("next")}
        />
      </div>
    </div>
...
export default function FlipbookViewer({
  ...
  const pageW = isMobile
    ? Math.min(rootW - 16, 380)
    : Math.floor((rootW - 60) / 2);
      ...
      >
        ...
        <div
          style={{
            ...
            background:
              "radial-gradient(ellipse at 20% 5%,rgba(255,120,120,0.08) 0%,transparent 55%)",
          }}
        />
            ...
            <div style={{ position: "relative", width: 64, height: 64 }}>
              <div
                style={{
                  ...
                  animation: "fb-spin 0.85s linear infinite",
                }}
              />
              <div
                style={{
            ...
            >
              <div
                style={{
                  height: "100%",
                  width: `${progress}%`,
                  background: "linear-gradient(90deg,#e63d3d,#ff8080)",
                  borderRadius: 2,
        ...
        {!loading && pages.length > 0 && (
          <div style={{ width: "100%" }}>
            <FlipEngine
              pages={pages}
              wrapperHeight={isFs ? (typeof window !== 'undefined' ? window.innerHeight : height) : height}
