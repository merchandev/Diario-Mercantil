...
export default function Historial(){
  ...
  return (
    <section className="space-y-4">
      ...
      {!loading && !error && (
        <>
      <div className="card p-3 text-sm space-y-2 text-slate-700">
        ...
        <div className="mt-2">
          <div className="font-semibold">Los estados de la solicitud son los siguientes:</div>
          <div className="mt-1 grid md:grid-cols-2 gap-2">
            <div><span className="font-semibold">Borrador:</span> Indica que el usuario completó los datos de la publicación, pero aún no ha adjuntado los archivos o reportado el pago. Mientras permanezca en este estado, no se genera la orden de servicio ni se inicia el proceso de publicación.</div>
            <div><span className="font-semibold">Por verificar:</span> La solicitud fue enviada y está pendiente la revisión de documentos y verificación del pago.</div>
            <div><span className="font-semibold">En trámite:</span> La publicación fue verificada y se encuentra en proceso para su publicación en el Diario.</div>
            <div><span className="font-semibold">Publicada:</span> La publicación ya se realizó y la edición correspondiente está disponible para descargar desde este mismo apartado.</div>
          </div>
          </div>
        </div>
              ...
              <tbody>
                {rows.map(r=> (
                  <tr key={r.id} className="border-t">
                    ...
                    <td className="px-4 py-2">{r.status==='Pendiente'?'Borrador':(r.status==='Publicado'?'Publicada':r.status)}</td>
