...
export default function Historial() {
  ...
  type Filters = { q: string; status: string; reqFrom: string; reqTo: string; pubFrom: string; pubTo: string }
  const load = (overrides?: Partial<Filters>) => {
    const filters: Filters = {
      q: overrides?.q ?? q,
      status: overrides?.status ?? status,
      reqFrom: overrides?.reqFrom ?? reqFrom,
      reqTo: overrides?.reqTo ?? reqTo,
      pubFrom: overrides?.pubFrom ?? pubFrom,
      pubTo: overrides?.pubTo ?? pubTo,
    }
    setLoading(true)
    setError(null)
    listLegal({ q: filters.q, status: filters.status === 'Todos' ? '' : filters.status, req_from: filters.reqFrom || undefined, req_to: filters.reqTo || undefined, pub_from: filters.pubFrom || undefined, pub_to: filters.pubTo || undefined })
      .then(r => {
        setRows(r.items)
      })
      .catch(() => {
        ...
        <>
          ...
          <div className="card p-3 grid md:grid-cols-4 gap-3">
            ...
            <select className="input" value={status} onChange={e => setStatus(e.target.value)}>
              {['Todos', 'Borrador', 'Por verificar', 'En trámite', 'Verificada', 'Publicada', 'Rechazado'].map(o => <option key={o} value={o}>{o}</option>)}
            </select>
            ...
            <div className="md:col-span-4 flex gap-2">
              ...
              <button className="btn" onClick={() => {
                setQ(''); setStatus('Todos'); setReqFrom(''); setReqTo(''); setPubFrom(''); setPubTo('')
                load({ q: '', status: 'Todos', reqFrom: '', reqTo: '', pubFrom: '', pubTo: '' })
              }}>Limpiar</button>
            </div>
          </div>
