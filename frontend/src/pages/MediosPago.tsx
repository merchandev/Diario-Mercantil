...
export default function MediosPago() {
  ...
  return (
    <section className="space-y-4">
      ...
      <form onSubmit={onCreate} className="card p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 items-end">
        <input className="input w-full" placeholder="Banco" value={form.bank || ''} onChange={e => setForm({ ...form, bank: e.target.value })} />
        <input className="input w-full" placeholder="Cuenta" value={form.account || ''} onChange={e => setForm({ ...form, account: e.target.value })} />
        <input className="input w-full" placeholder="Titular" value={form.holder || ''} onChange={e => setForm({ ...form, holder: e.target.value })} />
        <input className="input w-full" placeholder="RIF" value={form.rif || ''} onChange={e => setForm({ ...form, rif: e.target.value })} />
        <div className="flex flex-col sm:flex-row w-full gap-2 lg:col-span-1 sm:col-span-2 md:col-span-1">
          <input className="input w-full" placeholder="Teléfono" value={form.phone || ''} onChange={e => setForm({ ...form, phone: e.target.value })} />
