              ...
              <tr key={u.id} className="border-t">
                ...
                <td className="px-4 py-2 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <button className="p-1.5 text-slate-600 hover:bg-slate-50 rounded" onClick={() => setPromptDialog({ isOpen: true, title: 'Cambiar contraseña', message: 'Ingrese la nueva contraseña:', onConfirm: async (pw) => { if (!pw) return; try { await setUserPassword(u.id, pw); setAlertDialog({ isOpen: true, title: 'Éxito', message: 'Contraseña actualizada.', variant: 'success' }); } catch (e: any) { setAlertDialog({ isOpen: true, title: 'Error', message: e.message || 'No se pudo actualizar la contraseña', variant: 'error' }); } } })} title="Cambiar clave"><Key className="w-4 h-4" /></button>
            ...
            <form onSubmit={async e => {
              ...
              try {
                await updateUser(editUser.id, {
                  ...
                  person_type: editUser.person_type,
                })
