    ...
    private function commitPublication(int $editionId, int $actorId, array $expectedOrderIds): void
    {
        ...
        try {
            ...
            $ordersStmt = $this->pdo->prepare(
                'SELECT legal_request_id FROM edition_orders WHERE edition_id=? ORDER BY legal_request_id' . $lock
            );
            ...
            if (!(new EditionIntegrityService($this->pdo))->editionFileIsPublishable($edition)) {
                throw new RuntimeException('El PDF final no supera la validación de integridad y páginas.', 422);
            }
            ...
            $editionDate = (string) ($edition['date'] ?: EditorialClock::today());
            ...
            if ($lastPublished && $editionDate < $lastPublished) {
                throw new RuntimeException(
                    "La fecha de esta edición ({$editionDate}) debe ser mayor o igual a la última edición publicada ({$lastPublished}).",
                    400
                );
            }
            ...
            $publishedAt = EditorialClock::now()->format('Y-m-d H:i:s');
            $this->pdo->prepare(
